...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchTutors = async () => {
      try {
        const response = await axios.get(`http://${process.env.REACT_APP_BACKEND_BASE_URL}/api/student/getTutorsTeachingSubjects`, {
          headers: {
  ...
  const applyFilters = async () => {
    try {
      const response = await axios.get(`http://${process.env.REACT_APP_BACKEND_BASE_URL}/api/general/filterTutors`, {
        params: filters
      });
      setTutors(response.data);
    } catch (error) {
