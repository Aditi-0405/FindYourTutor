...
const ChatsListTutor = () => {
  ...
  useEffect(() => {
    const fetchChats = async () => {
      try {
        const response = await axios.get(`http://${process.env.REACT_APP_BACKEND_BASE_URL}/api/tutor/getMyChats`, {
          headers: {
        ...
        const studentsWithUnreadCounts = await Promise.all(response.data.map(async (student) => {
          const unreadResponse = await axios.get(`http://${process.env.REACT_APP_BACKEND_BASE_URL}/api/tutor/getIndividualNotifications/${student.studentId}`, {
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`,
            },
